...
This script:
  1. Loads a single-task LIBERO HDF5 file, preferably regenerated by
     regenerate_libero_filtered.py (successful demos, no no-op actions,
     224×224 simulator renders)
...
Both agentview and eye-in-hand images are rotated 180° (`img[::-1, ::-1]`)
before storage so training matches eval_libero.py.
...
def extract_chunks(
    ...
    for demo_i, dk in enumerate(demo_keys):
        ...
        for ci in range(n_chunks):
            ...
            samples["image_agent"].append(
                rotate_image_180(agent_imgs[t])
            )  # (H_img, W_img, 3) uint8
            samples["image_hand"].append(rotate_image_180(hand_imgs[t]))
            ...
            samples["image_agent_future"].append(rotate_image_180(agent_imgs[t_future]))
            samples["image_hand_future"].append(rotate_image_180(hand_imgs[t_future]))
