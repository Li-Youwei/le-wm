...
Validates the full pipeline with real preprocessed LIBERO data on GPU:
  LiberoDataset → DataLoader → JEPA(ViT + T5-small + ARPredictor) → L_CE → backward
...
def create_model(device: torch.device) -> JEPA:
    ...
    # ViT-Tiny encoder (shared for agentview + eye_in_hand)
...
def training_step(model: JEPA, batch: dict, device: torch.device) -> float:
    ...
    # 1. Encode (dual-view shared ViT + frozen T5)
    ...
    # 6. Verify gradients — all trainable components
    grad_checks = {
    ...
    print(f"  T5 frozen: confirmed (no gradients)")
...
def inference_test(model: JEPA, batch: dict, device: torch.device) -> None:
    ...
    tokens, lengths = model.predict_actions(
        ...
        max_len=MAX_ACTION_TOKENS, temperature=0.0,
    )
    ...
    real_mask = torch.arange(tokens.size(1), device=device).unsqueeze(0) < lengths.unsqueeze(1)
    if real_mask.any():
...
def vram_profile(model: JEPA, dataset: LiberoDataset, device: torch.device) -> None:
    ...
    for bs in [128, 64, 32, 16]:
        ...
        try:
            ...
            action_logits = model.predict(
                z_agent, z_hand, batch["proprio"], lang_embeds, lang_lengths,
                batch["fast_tokens"], batch["fast_lengths"],
            )
...
def main() -> None:
    ...
    for key, expected_shape in checks.items():
        ...
        assert actual == expected_shape, f"{key}: expected {expected_shape}, got {actual}"
    ...
    for i, batch in enumerate(loader):
        ...
        B = batch["pixels_agent"].size(0)
        print(f"\n--- Batch {i} (B={B}) ---")
        print(f"  fast_lengths = {batch['fast_lengths'].tolist()[:8]}{'...' if B > 8 else ''}")
