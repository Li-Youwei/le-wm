...
Two sources, selected by ``cfg.vision_encoder.source``:

- ``spt_vit`` (default): the repo's original ``spt.backbone.utils.vit_hf``
  factory. With ``pretrained=false, freeze=false`` this is bit-identical to
  the frozen baseline encoder (random-init trainable ViT-Tiny).
- ``hf``: ``AutoModel.from_pretrained(model_name_or_path)`` returning
  ``last_hidden_state`` — e.g. a frozen DINOv2 (``facebook/dinov2-small``).
  Wrapped in ``HFVisionBackbone`` so the JEPA call site stays stable, and
  frozen (``eval()`` + ``requires_grad_(False)``) when ``freeze=true``.

Ported from the codex branch's ``vision_backbone.py`` (the frozen-HF-backbone
mechanism) so claude/clever-black gains frozen-DINOv2 support without changing
the V17 / full-MoT / SP+SIGReg design.
"""
...
class HFVisionBackbone(nn.Module):
    ...
    def forward(
        self, pixel_values: torch.Tensor, interpolate_pos_encoding: bool = True
    ):
        try:
            out = self.model(
...
def build_visual_encoder(cfg: Any, spt_module: Any) -> tuple[nn.Module, int, bool]:
    ...
    if source == "spt_vit":
        ...
        return (
            ...
            bool(_cfg_get(vision_cfg, "freeze", False)),
        )
    ...
    local_files_only = bool(
        _cfg_get(vision_cfg, "local_files_only", _offline_default())
    )
