...
| Mixture-of-Transformers  | `predictor.use_mot`       | `false` | Per-modality FFN experts (prefix/action/state-query); attention shared; predictor ~1.6–2× params               |
...
- **SP:** three STATE_QUERY tokens `Q_ag, Q_hd, Q_pr` appended at the very end.
...
`use_mot=true`: each `Block` swaps `FeedForward`→`MoTFeedForward`, one expert per modality (attention shared). Hard partition by absolute position: M=0 prefix, M=1 action zone, M=2 state-query (SP only). `n_modalities` = 3 w/ SP, 2 without. `MoTFeedForward` runs every expert on the full sequence + merges by modality mask (~2× FFN cost, chosen for code simplicity). All three forward paths build their own `_build_modality_ids (B,L)`. `use_mot=false` → original shared MLP, `modality_ids=None`, bit-identical to `7008f15`. Predictor params: 11.92M (MoT off, SP on) → 21.39M (MoT+SP, +79%) / 14.65M (MoT, no SP, +23%).
...
`nv=n_visual_per_view` (1 CLS-only, 17 under V17). nv=1 is bit-identical to the old arch. Predictor params under SP: 11.92M (nv=1) → 24.52M (nv=17).
...
2. Needs batch ≥ 32 (SIGReg moments + BN stats). **`overfit.yaml` defaults batch=8 — too small for SP+SIGReg; override `loader.batch_size=32`.** Don't go below 32.
3. Multi-GPU needs `SyncBatchNorm` (not auto-converted in `train.py` — flag when adding multi-GPU).
...
| `module.py`                                | `ARPredictor`, `Block`, `Attention`, `MLP`, `FeedForward`, `MoTFeedForward`, `SIGReg`; vocab consts (BOS 1024 / EOS 1025 / PAD 1026 / TOTAL 1027 / HEAD 1026). All toggles default off.                                |
...
python train.py data=libero +visual_tokens.pool_grid=4
